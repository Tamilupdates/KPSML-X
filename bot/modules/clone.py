...
async def gdcloneNode(message, link, listen_up):
    ...
    if is_gdrive_link(link):
        ...
        if org_link:
            cget().request('POST', "https://kpsmlcontribute.vercel.app/contribute", headers={"Content-Type": "application/json"}, data=jdumps({"name": name, "link": org_link, "size": get_readable_file_size(size)}))
